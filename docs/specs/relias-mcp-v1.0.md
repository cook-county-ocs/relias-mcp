...
1. **Refresh-token rotation behavior — CONFIRMED ROTATING (2026-05-26).** Relias issues one-time refresh tokens: each `refresh_token` grant returns a new token and invalidates the old (verified via `scripts/check-rotation.mjs`). F1's `OidcAuth` rotates in-memory and exposes the latest token via `currentRefreshToken`. **P7's cron MUST persist that rotated token back to the `RELIAS_OIDC_REFRESH_TOKEN` secret after every run** (e.g. `gh secret set`, using the Secrets:write PAT from chore **C14**), persisting immediately after the refresh and before the catalog pull so a later failure can't strand a consumed-but-unsaved token. If a run fails to persist, recovery is re-running the C7 bootstrap (~10 min).
